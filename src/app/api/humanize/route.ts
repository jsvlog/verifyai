// POST /api/humanize — Paid feature: requires active subscription
import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import { humanizeText } from '@/lib/detection';
  ...
  try {
    ...
    if (!user) {
      return NextResponse.json({
        success: false,
        error: 'Humanizer requires an account. Please sign up to continue.',
        code: 'AUTH_REQUIRED',
        requestId: genId(),
      }, { status: 401 });
    }
    ...
    if (!isPaid) {
      return NextResponse.json({
        success: false,
        error: 'Text Humanizer is a premium feature. Upgrade your plan to unlock.',
    ...
    // --- Process ---
    const result = await humanizeText(text.trim());

    // Save scan
    await adminClient.from('scans').insert({
      user_id: user.id, type: 'humanizer', input_text: text.trim(),
